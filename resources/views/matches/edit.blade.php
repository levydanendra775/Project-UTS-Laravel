...
<div style="margin-bottom: 32px;">
    ...
    <h1 style="margin-top: 12px;">Edit Jadwal Pertandingan</h1>
...
<div class="card" style="max-width: 700px; margin: 0;">
    <h3 style="margin-bottom: 24px; border-bottom: 1px solid var(--border-color); padding-bottom: 12px;">
        <i class="fa-solid fa-calendar-plus" style="color: var(--secondary);"></i> Perbarui Jadwal Pertandingan
    </h3>
    ...
    <form action="{{ route('matches.update', $match->id) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <select name="round" id="round" class="form-control" style="background-color: var(--bg-sidebar); cursor: pointer;" required onchange="toggleGroupSelect()">
                <option value="group" {{ old('round', $match->round) === 'group' ? 'selected' : '' }}>Babak Penyisihan Grup</option>
                <option value="quarterfinal" {{ old('round', $match->round) === 'quarterfinal' ? 'selected' : '' }}>Perempat Final (Quarter-final)</option>
                <option value="semifinal" {{ old('round', $match->round) === 'semifinal' ? 'selected' : '' }}>Semifinal</option>
                <option value="final" {{ old('round', $match->round) === 'final' ? 'selected' : '' }}>Final</option>
            </select>
            @error('round')
        ...
        <div class="form-group" id="group_select_container">
            ...
            <select name="group_id" id="group_id" class="form-control" style="background-color: var(--bg-sidebar); cursor: pointer;">
                ...
                @foreach($groups as $group)
                    <option value="{{ $group->id }}" {{ old('group_id', $match->group_id) == $group->id ? 'selected' : '' }}>
                        {{ $group->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group">
            ...
            <input type="datetime-local" name="match_date" id="match_date" class="form-control" value="{{ old('match_date', $match->match_date->format('Y-m-d\TH:i')) }}" required>
            @error('match_date')
                <span class="form-error">{{ $message }}</span>
            @enderror
        </div>
